...
public class ListarVacuna extends JDialog {
    ...
    public ListarVacuna() {
        ...
        String[] header = { "Código", "Vacuna", "Enfermedades que cura (Cantidad)" };
        model = new DefaultTableModel();
        model.setColumnIdentifiers(header);
        table = new JTable();
        table.setSelectionMode(ListSelectionModel.SINGLE_SELECTION);
        ...
        btnVerMas.addActionListener(new ActionListener() {
        	public void actionPerformed(ActionEvent e) {
        		 ...
        		 if (selectedRow != -1) {
                     ...
                     VerMasEnfermedadesVacuna verMas = new VerMasEnfermedadesVacuna(vacuna);
                     verMas.setModal(true);
